...
/**
 * Migration v12 -> v13: Add 'hivemind' to conversations type CHECK constraint
 */
const migration_v13: IMigration = {
  version: 13,
  name: 'Add hivemind to conversations type constraint',
  up: (db) => {
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_new (
        id TEXT PRIMARY KEY,
    ...
    console.log('[Migration v13] Added hivemind to conversations type constraint');
  },
  down: (db) => {
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_rollback (
      ...
      INSERT INTO conversations_rollback (id, user_id, name, type, extra, model, status, source, created_at, updated_at)
      SELECT id, user_id, name, type, extra, model, status, source, created_at, updated_at
      FROM conversations
      WHERE type != 'hivemind';
    ...
    console.log('[Migration v13] Rolled back: Removed hivemind from conversations type constraint');
  },
};
...
const migration_v14: IMigration = {
  ...
  up: (db) => {
    ...
    console.log('[Migration v14] Added Agent Teams tables');
  },
  down: (db) => {
    db.exec(`
      DROP TABLE IF EXISTS agent_sessions;
      DROP TABLE IF EXISTS agent_team_messages;
      DROP TABLE IF EXISTS agent_task_dependencies;
      DROP TABLE IF EXISTS agent_tasks;
      DROP TABLE IF EXISTS agent_teammates;
      DROP TABLE IF EXISTS agent_teams;
    `);

    console.log('[Migration v14] Rolled back: Removed Agent Teams tables');
  },
};
