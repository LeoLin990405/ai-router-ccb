...
export async function resetPasswordCLI(username: string): Promise<void> {
  ...
  try {
    ...
    const dbPath = path.join(getDataPath(), 'hivemind.db');
    ...
    if (!tableExists) {
      ...
      log.info('');
      log.info('Please run HiveMind at least once to initialize the database:');
      log.info('  hivemind --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  hivemind --resetpass <username>');
      process.exit(1);
    }
