...
/**
 * 从 MODEL_REGISTRY 动态构建 ACP Provider 模型列表（仅多模型 Provider）
 * Dynamically build ACP provider model map from MODEL_REGISTRY (multi-model providers only)
 */
const ACP_PROVIDER_MODELS: Record<string, { id: string; label: string; description?: string; isDefault?: boolean }[]> = (() => {
  const result: Record<string, { id: string; label: string; description?: string; isDefault?: boolean }[]> = {};
  for (const entry of MODEL_REGISTRY) {
    // 跳过 Gemini（有自己的选择器）和单模型 Provider
    if (entry.provider === 'gemini' || entry.models.length <= 1) continue;
    result[entry.provider] = entry.models.map((m) => ({
      id: m.id,
      label: m.displayName,
...
const OBSIDIAN_HIVEMIND_AGENT_KEY = 'hivemind-obsidian';
...
const HIVEMIND_AGENT_OPTIONS: Array<{
  ...
  { key: HIVEMIND_AGENT_KEY, label: 'Hivemind', tokens: ['hivemind', 'hive', 'scene', 'single', 'chat'], emoji: '🐝' },
  { key: OBSIDIAN_HIVEMIND_AGENT_KEY, label: 'Obsidian CLI', tokens: ['obsidian', 'vault', 'knowledge'], emoji: '📚' },
];
...
const isObsidianHivemindAgentKey = (key: string): key is typeof OBSIDIAN_HIVEMIND_AGENT_KEY => {
  return key === OBSIDIAN_HIVEMIND_AGENT_KEY;
};
...
const Guid: React.FC = () => {
  ...
  const [mentionSelectorVisible, setMentionSelectorVisible] = useState(true);
  ...
  // ACP 模型选择状态 / ACP model selection state
  ...
  const [selectedAgentKey, _setSelectedAgentKey] = useState<string>(HIVEMIND_AGENT_KEY);
  const routeAgentKey = useMemo(() => {
    const params = new URLSearchParams(location.search);
    const agent = params.get('agent');
    if (agent === HIVEMIND_AGENT_KEY || agent === OBSIDIAN_HIVEMIND_AGENT_KEY) {
      return agent;
    }
    return null;
  }, [location.search]);
  ...
  useEffect(() => {
    if (!routeAgentKey) return;
    setSelectedAgentKey(routeAgentKey);
  }, [routeAgentKey, setSelectedAgentKey]);
  ...
  const selectedAgent = selectedAgentKey.startsWith('custom:') ? 'custom' : isHivemindAgentKey(selectedAgentKey) || isObsidianHivemindAgentKey(selectedAgentKey) ? HIVEMIND_AGENT_KEY : (selectedAgentKey as AcpBackend);
  ...
  // 判断是否选择了 ACP 后端（非 Gemini）
  // Check if an ACP backend is selected (not Gemini)
  const selectedAcpBackend = useMemo(() => {
    if (!selectedAgentInfo) return null;
    const backend = selectedAgentInfo.backend;
    // 排除 Gemini 和 hivemind
    if (backend === 'gemini' || backend === 'hivemind') return null;
    return backend as AcpBackend;
  }, [selectedAgentInfo]);

  // 动态模型列表（从 CLI 配置扫描获取，覆盖静态列表）
  // Dynamic model list (scanned from CLI configs, overrides static list)
  const [dynamicModels, setDynamicModels] = useState<Record<string, { id: string; label: string; description?: string; isDefault?: boolean }[]>>({});

  // 当前 Provider 的模型列表：优先动态，回退静态
  // Current provider's model list: prefer dynamic, fallback to static
  const currentAcpModels = useMemo(() => {
    if (!selectedAcpBackend) return [];
    return dynamicModels[selectedAcpBackend] || ACP_PROVIDER_MODELS[selectedAcpBackend] || [];
  }, [selectedAcpBackend, dynamicModels]);

  // 当 ACP backend 改变时，立即显示静态模型，然后异步加载动态模型
  // On backend change: show static models immediately, then async load dynamic ones
  useEffect(() => {
    if (!selectedAcpBackend) {
      setSelectedAcpModel(null);
      return;
    }

    // 立即从静态列表选择默认模型
    const staticModels = ACP_PROVIDER_MODELS[selectedAcpBackend] || [];
    if (staticModels.length > 0) {
      const defaultModel = staticModels.find((m) => m.isDefault) || staticModels[0];
      if (defaultModel) setSelectedAcpModel(defaultModel.id);
    }
    ...
    Promise.race([
      ipcBridge.models.getModels.invoke({ provider: selectedAcpBackend }).catch(() => null),
      timeout,
    ]).then((models) => {
      if (cancelled || !Array.isArray(models) || models.length === 0) return;
      // 转换为 dropdown 格式
      const formatted = models.map((m: { id: string; displayName?: string; description?: string; isDefault?: boolean }) => ({
        id: m.id,
        label: m.displayName || m.id,
        description: m.description,
        isDefault: m.isDefault,
      }));
      setDynamicModels((prev) => ({ ...prev, [selectedAcpBackend]: formatted }));
      // 如果当前选中的模型不在新列表中，选择默认
      const currentValid = formatted.some((m) => m.id === selectedAcpModel);
      if (!currentValid) {
        const defaultModel = formatted.find((m) => m.isDefault) || formatted[0];
        if (defaultModel) setSelectedAcpModel(defaultModel.id);
      }
    });
  ...
  // 当前选中的 ACP 模型显示名称
  // Display label for selected ACP model
  const selectedAcpModelLabel = useMemo(() => {
    if (!selectedAcpBackend || !selectedAcpModel) return null;
    return currentAcpModels.find((m) => m.id === selectedAcpModel)?.label || selectedAcpModel;
  }, [selectedAcpBackend, selectedAcpModel, currentAcpModels]);
  ...
  const selectedAgentLabel = selectedAgentInfo?.name || mentionOptions.find((option) => option.key === selectedAgentKey)?.label || selectedAgentKey;
  ...
  useEffect(() => {
    ...
    const loadLastSelectedAgent = async () => {
      try {
        ...
        if (isHivemindAgentKey(savedAgentKey) || isObsidianHivemindAgentKey(savedAgentKey) || isDirectAgentKey(savedAgentKey)) {
          _setSelectedAgentKey(savedAgentKey);
          return;
        }
  ...
  const currentEffectiveAgentInfo = useMemo(() => {
    if (!isPresetAgent) {
      ...
      if (selectedAgent === HIVEMIND_AGENT_KEY) {
        return { agentType: 'gemini' as PresetAgentType, isFallback: false, originalType: 'gemini' as PresetAgentType, isAvailable: true };
      }
  ...
  const handleSend = async () => {
    ...
    if (!selectedAgent || selectedAgent === 'gemini' || (isPreset && finalEffectiveAgentType === 'gemini')) {
      ...
      return;
    } else if (isHivemindAgentKey(selectedAgentKey) || isObsidianHivemindAgentKey(selectedAgentKey)) {
      try {
        if (isHivemindAgentKey(selectedAgentKey)) {
          sessionStorage.setItem(
            AGENT_TEAMS_CHAT_INITIAL_MESSAGE_KEY,
            JSON.stringify({
              input,
            }),
          );
          await navigate('/agent-teams/chat');
          return;
        }
        ...
        if (!conversation || !conversation.id) {
          alert('Failed to create Hivemind conversation.');
          return;
        }
        ...
        const initialMessage = {
          input,
          files: files.length > 0 ? files : undefined,
          ...(isObsidianHivemindAgentKey(selectedAgentKey) ? { provider: 'obsidian' } : {}),
        };
      ...
      try {
        ...
        const conversation = await ipcBridge.conversation.create.invoke({
          ...
          extra: {
            ...
            // 选择的模型 ID / Selected model ID
            modelId: selectedAcpModel || undefined,
          },
        });
        ...
        const initialMessage = {
          input,
          files: files.length > 0 ? files : undefined,
          model: typeof currentModel?.useModel === 'string' ? currentModel.useModel : undefined,
        };
  ...
  return (
    <ConfigProvider getPopupContainer={() => guidContainerRef.current || document.body}>
      <div ref={guidContainerRef} className={`${styles.guidContainer} nexus-guid-page`}>
        <div className={styles.guidLayout}>
          ...
          {/* Agent 选择器 - 在标题下方 */}
          {
            <div className='w-full flex justify-center'>
              <div
                className='nexus-guid-agent-rail inline-flex items-center bg-fill-2'
                style={{
                  marginBottom: 16,
                  padding: '4px',
                  borderRadius: '30px',
          ...
          <div
            className={`${styles.guidInputCard} nexus-guid-input-card relative p-16px border-3 b bg-dialog-fill-0 b-solid rd-20px flex flex-col ${mentionOpen ? 'overflow-visible' : 'overflow-hidden'} transition-all duration-200 ${isFileDragging ? 'border-dashed' : ''}`}
            style={{
              zIndex: 1,
          ...
          >
            {mentionSelectorVisible && (
              <div className='flex items-center gap-8px mb-8px'>
                ...
                >
                  <div className='nexus-guid-mention-pill flex items-center gap-6px bg-fill-2 px-10px py-4px rd-16px cursor-pointer select-none'>
            ...
            <div className={styles.actionRow}>
              <div className={styles.actionTools}>
                ...
                {/* ACP 模型选择器 - 与 Gemini 同样的 Dropdown 样式 */}
                {selectedAcpBackend && currentAcpModels.length > 0 && (
                  <Dropdown
                    trigger='hover'
                    droplist={
                      <Menu selectedKeys={selectedAcpModel ? [selectedAcpModel] : []}>
                        {currentAcpModels.map((model) => (
                          <Menu.Item
                            key={model.id}
                            className={selectedAcpModel === model.id ? '!bg-2' : ''}
                            onClick={() => setSelectedAcpModel(model.id)}
                          >
                            <div className='flex items-center gap-8px'>
                              <span>{model.label}</span>
                              {model.isDefault && (
                                <span className='text-10px px-4px py-1px bg-primary/10 text-primary rd-4px'>默认</span>
                              )}
                            </div>
                            {model.description && (
                              <div className='text-11px text-t-tertiary mt-2px'>{model.description}</div>
                            )}
                          </Menu.Item>
                        ))}
                      </Menu>
                  ...
                  >
                    <Button className={'sendbox-model-btn'} shape='round'>
                      {selectedAcpModelLabel || t('conversation.welcome.selectModel')}
                    </Button>
                  </Dropdown>
                )}
                ...
                {isPresetAgent && selectedAgentInfo && (
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedAgentKey(HIVEMIND_AGENT_KEY); // Reset to default (Hivemind)
                      }}
                    >
        ...
        <div className='absolute bottom-32px left-50% -translate-x-1/2 flex flex-col justify-center items-center'>
          ...
          <div className='flex justify-center items-center gap-24px'>
            <div className='group flex items-center justify-center w-36px h-36px rd-50% bg-fill-0 cursor-pointer overflow-hidden whitespace-nowrap hover:w-200px hover:rd-28px hover:px-20px hover:justify-start hover:gap-10px transition-all duration-400 ease-[cubic-bezier(0.2,0.8,0.3,1)]' style={quickActionStyle(hoveredQuickAction === 'feedback')} onMouseEnter={() => setHoveredQuickAction('feedback')} onMouseLeave={() => setHoveredQuickAction(null)} onClick={() => openLink('https://x.com/HiveMind')}>
            ...
            <div className='group flex items-center justify-center w-36px h-36px rd-50% bg-fill-0 cursor-pointer overflow-hidden whitespace-nowrap hover:w-200px hover:rd-28px hover:px-20px hover:justify-start hover:gap-10px transition-all duration-400 ease-[cubic-bezier(0.2,0.8,0.3,1)]' style={quickActionStyle(hoveredQuickAction === 'repo')} onMouseEnter={() => setHoveredQuickAction('repo')} onMouseLeave={() => setHoveredQuickAction(null)} onClick={() => openLink('https://github.com/iOfficeAI/HiveMind')}>
