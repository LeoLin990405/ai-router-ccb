...
export interface AcpBackendConfig {
   ...
   * 不同 CLI 使用不同的模型选择参数：
   *   - '-m' 用于 qwen, kimi（短参数）
   *   - '--model' 用于 codex, goose 等
   *   - undefined 表示不支持 CLI 级别模型选择（通过 ACP 协议选择）
   *
   * Model selection argument format.
   * Different CLIs use different model selection arguments:
   *   - '-m' for qwen, kimi (short flag)
   *   - '--model' for codex, goose, etc.
   *   - undefined means CLI doesn't support model selection (use ACP protocol instead)
   * When specified, the model ID will be appended as the next argument.
   * Example: modelArg: '-m' results in ['-m', 'model-id'] in CLI args
   */
  modelArg?: string;
...
export const ACP_BACKENDS_ALL: Record<AcpBackendAll, AcpBackendConfig> = {
  ...
  qwen: {
    ...
    supportsStreaming: true,
    acpArgs: ['--acp', '--auth-type', 'qwen-oauth'], // Use --acp with OAuth auth
    modelArg: '-m', // Qwen supports -m flag for model selection
  },
  iflow: {
    id: 'iflow',
    name: 'iFlow CLI',
    ...
    enabled: true,
    supportsStreaming: false,
    modelArg: '-m', // iFlow supports -m flag for model selection
  },
  ollama: {
    id: 'ollama',
    name: 'Ollama',
  ...
  codex: {
    ...
    supportsStreaming: false,
    modelArg: '--model', // Codex supports --model flag
  },
  goose: {
    id: 'goose',
    name: 'Goose',
    ...
    modelArg: '--model', // Goose supports --model flag
  },
  auggie: {
    id: 'auggie',
    name: 'Augment Code',
    ...
    modelArg: '--model', // Auggie supports --model flag
  },
  kimi: {
    id: 'kimi',
    name: 'Kimi CLI',
    ...
    modelArg: '-m', // Kimi supports -m flag for model selection
  },
  opencode: {
    id: 'opencode',
    name: 'OpenCode',
    ...
    modelArg: '-m', // OpenCode supports -m flag for model selection
  },
  copilot: {
    id: 'copilot',
    name: 'GitHub Copilot',
    ...
    modelArg: '--model', // Copilot supports --model flag
  },
  qoder: {
    id: 'qoder',
    name: 'Qoder CLI',
