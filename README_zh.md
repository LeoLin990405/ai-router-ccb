<p align="center">
  <img src="https://img.shields.io/badge/AI%20Orchestrator-多AI协作平台-blue?style=for-the-badge" alt="AI Orchestrator">
  <img src="https://img.shields.io/badge/Provider-7个-green?style=for-the-badge" alt="Providers">
  <img src="https://img.shields.io/badge/Gateway-REST%20%2B%20WebSocket-orange?style=for-the-badge" alt="Gateway">
</p>

<h1 align="center">AI Orchestrator</h1>

<p align="center">
  <strong>企业级多 AI 编排平台</strong>
  <br>
  <em>Claude 作为主脑，通过统一 Gateway API 调度 7 个 AI Provider</em>
</p>

<p align="center">
  <a href="#-特性">特性</a> •
  <a href="#-快速开始">快速开始</a> •
  <a href="#-架构">架构</a> •
  <a href="#-gateway-api">Gateway API</a> •
  <a href="#-phase-7-功能">Phase 7</a> •
  <a href="#-安装">安装</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Python-3.9+-3776AB?logo=python&logoColor=white" alt="Python">
  <img src="https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white" alt="FastAPI">
  <img src="https://img.shields.io/badge/SQLite-003B57?logo=sqlite&logoColor=white" alt="SQLite">
  <img src="https://img.shields.io/badge/Prometheus-E6522C?logo=prometheus&logoColor=white" alt="Prometheus">
  <img src="https://img.shields.io/badge/License-MIT-yellow" alt="License">
</p>

[English](README.md) | **中文**

---

## 🎯 概述

**AI Orchestrator** 是一个生产级多 AI 编排平台，**Claude 作为主脑（Orchestrator）**，通过统一的 Gateway API 智能调度 7 个 AI Provider。

### 架构理念

```
┌─────────────────────────────────────────────────────────┐
│              Claude (主脑 / Orchestrator)                │
│                   Claude Code CLI                        │
└─────────────────────┬───────────────────────────────────┘
                      │ 任务调度
                      ▼
┌─────────────────────────────────────────────────────────┐
│                   CCB Gateway API                        │
│                 http://localhost:8765                    │
└─────────────────────┬───────────────────────────────────┘
                      │
        ┌─────────────┼─────────────┐
        ▼             ▼             ▼
   ┌─────────┐  ┌─────────┐  ┌─────────┐
   │ Gemini  │  │ DeepSeek│  │  Codex  │
   └─────────┘  └─────────┘  └─────────┘
   ┌─────────┐  ┌─────────┐  ┌─────────┐
   │OpenCode │  │  Kimi   │  │  Qwen   │
   └─────────┘  └─────────┘  └─────────┘
   ┌─────────┐
   │  iFlow  │
   └─────────┘
```

### 为什么选择 AI Orchestrator？

| 挑战 | 解决方案 |
|------|----------|
| 多个 AI CLI 接口不统一 | **统一 Gateway API**，所有 Provider 一致 |
| 手动选择 Provider | **智能路由**，基于任务分析自动选择 |
| 无法观察 AI 操作 | **实时监控**，WebSocket 事件推送 |
| 无缓存或重试逻辑 | **内置缓存、重试和降级** |
| 无限流或认证 | **API Key 认证 + 令牌桶限流** |

---

## ✨ 特性

### Gateway API（核心）

| 特性 | 描述 |
|------|------|
| **REST API** | `POST /api/ask`, `GET /api/reply/{id}`, `GET /api/status` |
| **WebSocket** | 实时事件推送 `/api/ws` |
| **优先级队列** | SQLite 持久化的请求优先级队列 |
| **多后端** | HTTP API、CLI 执行、WezTerm 集成 |
| **健康监控** | 自动 Provider 健康检查和指标 |

### Phase 7 生产级功能

| 特性 | 描述 |
|------|------|
| **API 认证** | 基于 API Key 的认证，SHA-256 哈希 |
| **限流** | 令牌桶算法，支持按 Key 限流 |
| **响应缓存** | SQLite 缓存，支持 TTL 和模式排除 |
| **重试与降级** | 指数退避，自动 Provider 降级 |
| **并行查询** | 同时查询多个 Provider |
| **Prometheus 指标** | `/metrics` 端点用于监控 |
| **流式传输** | Server-Sent Events 实时响应 |

---

## 🚀 快速开始

### 启动 Gateway

```bash
# 启动 Gateway 服务器
cd ~/.local/share/codex-dual
python3 -m lib.gateway.gateway_server --config ~/.ccb/gateway.yaml

# 或使用 shell 函数
source ~/.ccb/gateway-functions.sh
gw-start
```

### 发送请求

```bash
# 通过 REST API
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{"provider": "qwen", "message": "你好"}'

# 获取响应
curl http://localhost:8765/api/reply/{request_id}?wait=true

# 通过 shell 函数
source ~/.ccb/gateway-functions.sh
gw-ask qwen "你的问题"
gw-kimi "你的问题"
gw-parallel "查询所有 Provider" first_success
```

### 检查状态

```bash
# Gateway 状态
curl http://localhost:8765/api/status

# 缓存统计
curl http://localhost:8765/api/cache/stats

# Prometheus 指标
curl http://localhost:8765/metrics
```

---

## 🏗️ 架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        AI Orchestrator 架构                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                    Claude (主脑 / Orchestrator)                        │ │
│  │                    智能任务调度与协调                                   │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                         Gateway API 层                                 │ │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌─────────┐ │ │
│  │  │ REST API  │ │ WebSocket │ │   认证    │ │   限流    │ │  指标   │ │ │
│  │  │ (FastAPI) │ │  (事件)   │ │ (API Key) │ │(令牌桶)   │ │(Prometh)│ │ │
│  │  └───────────┘ └───────────┘ └───────────┘ └───────────┘ └─────────┘ │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                         处理层                                         │ │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐             │ │
│  │  │   缓存    │ │   重试    │ │   并行    │ │   流式    │             │ │
│  │  │ (SQLite)  │ │  (降级)   │ │ (多AI)    │ │  (SSE)    │             │ │
│  │  └───────────┘ └───────────┘ └───────────┘ └───────────┘             │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                      Provider 层 (7 个 Provider)                       │ │
│  │  ┌───────┐ ┌───────┐ ┌────────┐ ┌────────┐ ┌───────┐ ┌───────┐      │ │
│  │  │Gemini │ │DeepSeek│ │ Codex │ │OpenCode│ │ Kimi  │ │ Qwen  │      │ │
│  │  └───────┘ └───────┘ └────────┘ └────────┘ └───────┘ └───────┘      │ │
│  │  ┌───────┐                                                           │ │
│  │  │ iFlow │                                                           │ │
│  │  └───────┘                                                           │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🌐 Gateway API

### REST 端点

| 方法 | 端点 | 描述 |
|------|------|------|
| `POST` | `/api/ask` | 向 Provider 提交请求 |
| `GET` | `/api/reply/{request_id}` | 获取响应（支持 `?wait=true`） |
| `GET` | `/api/status` | 获取 Gateway 和 Provider 状态 |
| `GET` | `/api/requests` | 列出最近请求 |
| `DELETE` | `/api/request/{request_id}` | 取消待处理请求 |
| `GET` | `/api/health` | 健康检查 |
| `GET` | `/api/cache/stats` | 缓存统计 |
| `DELETE` | `/api/cache` | 清除缓存 |
| `GET` | `/metrics` | Prometheus 指标 |
| `GET` | `/docs` | 交互式 API 文档 |

### 请求示例

```bash
# 提交请求
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{
    "provider": "qwen",
    "message": "解释 Python 中的 async/await",
    "timeout_s": 60,
    "priority": 50
  }'

# 响应
{
  "request_id": "abc123-def",
  "provider": "qwen",
  "status": "queued",
  "cached": false,
  "parallel": false
}

# 并行查询所有 Provider
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{
    "provider": "@all",
    "message": "2+2=?",
    "aggregation_strategy": "first_success"
  }'
```

---

## 🔧 Phase 7 功能

### API 认证

```bash
# 创建 API Key
curl -X POST http://localhost:8765/api/admin/keys \
  -H "Content-Type: application/json" \
  -d '{"name": "my-app", "rate_limit_rpm": 100}'

# 使用 API Key
curl -X POST http://localhost:8765/api/ask \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"provider": "qwen", "message": "你好"}'
```

### 限流

- 令牌桶算法
- 默认：60 请求/分钟
- 支持按 Key 自定义限流
- 支持突发流量

### 响应缓存

```bash
# 查看缓存统计
curl http://localhost:8765/api/cache/stats

# 响应
{
  "hits": 15,
  "misses": 45,
  "hit_rate": 0.25,
  "total_entries": 30,
  "expired_entries": 5
}

# 清除缓存
curl -X DELETE http://localhost:8765/api/cache
```

### Prometheus 指标

```bash
curl http://localhost:8765/metrics

# 输出
gateway_requests_total{provider="qwen",status="completed"} 150
gateway_request_latency_seconds_bucket{provider="qwen",le="5.0"} 120
gateway_cache_hits_total 15
gateway_cache_misses_total 45
```

---

## 📦 Provider

### Provider 矩阵

| Provider | 命令 | 后端 | 最佳用途 | 状态 |
|----------|------|------|----------|------|
| **Gemini** | `gw-gemini` | WezTerm | 前端、审查 | ✅ |
| **DeepSeek** | `gw-deepseek` | CLI 执行 | 深度推理 | ✅ |
| **Codex** | `gw-codex` | CLI 执行 | 后端、API | ✅ |
| **OpenCode** | `gw-opencode` | CLI 执行 | 通用编码 | ✅ |
| **Kimi** | `gw-kimi` | CLI 执行 | 中文、长上下文 | ✅ |
| **Qwen** | `gw-qwen` | CLI 执行 | 多语言 | ✅ |
| **iFlow** | `gw-iflow` | CLI 执行 | 工作流 | ✅ |

> **注意**：Claude 是主脑（Orchestrator），不参与任务调度。

---

## 🔧 安装

### 前置条件

- **Python 3.9+**
- **WezTerm**（推荐）或 tmux
- Provider CLI：`codex`, `gemini`, `opencode`, `deepseek`, `kimi`, `qwen`, `iflow`

### 安装

```bash
# 克隆仓库
git clone https://github.com/LeoLin990405/ai-router-ccb.git ~/.local/share/codex-dual

# 安装依赖
pip install fastapi uvicorn pyyaml aiohttp prometheus-client

# 添加到 PATH
export PATH="$HOME/.local/share/codex-dual/bin:$PATH"

# 加载 shell 函数
echo 'source ~/.ccb/gateway-functions.sh' >> ~/.zshrc
```

### 配置

```yaml
# ~/.ccb/gateway.yaml
server:
  host: "127.0.0.1"
  port: 8765

default_provider: "qwen"

providers:
  gemini:
    enabled: true
    backend_type: "cli_exec"
    cli_command: "gemini"
    timeout_s: 300
  # ... 其他 provider
```

---

## 📊 性能

### Provider 延迟（典型值）

| Provider | 平均延迟 | 用途 |
|----------|----------|------|
| Kimi | ~9s | 中文内容 |
| Qwen | ~14s | 多语言 |
| iFlow | ~17s | 工作流 |
| Codex | ~19s | 代码生成 |
| OpenCode | ~27s | 通用编码 |
| Gemini | ~31s | 前端、审查 |
| DeepSeek | ~47s | 深度推理 |

---

## 🙏 致谢

- **[bfly123/claude_code_bridge](https://github.com/bfly123/claude_code_bridge)** - 原始多 AI 协作框架
- **[Grafbase/Nexus](https://github.com/grafbase/nexus)** - AI 网关架构灵感

---

## 👥 贡献者

- **Leo** ([@LeoLin990405](https://github.com/LeoLin990405)) - 项目负责人
- **Claude** (Anthropic Claude Opus 4.5) - 架构与实现

---

## 📄 许可证

MIT License - 见 [LICENSE](LICENSE)

---

<p align="center">
  <sub>人与 AI 协作构建</sub>
  <br>
  <sub>⭐ 如果觉得有用，请给个 Star！</sub>
</p>
