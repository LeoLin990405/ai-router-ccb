...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".deepseek-session")
...
def load_project_session(work_dir: Path) -> Optional[DeepseekProjectSession]:
    ...
    if data.get("active") is False:
        return None
    return DeepseekProjectSession(session_file=session_file, data=data)


def compute_session_key(session: DeepseekProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"deepseek:{pid}" if pid else "deepseek:unknown"
