...
def _ready_markers() -> list[str]:
    raw = (os.environ.get("CCB_GASKD_READY_MARKERS") or "").strip()
    if raw:
        parts = [p.strip() for p in raw.split(",") if p.strip()]
        return parts
    # Default markers seen in Gemini CLI startup banners/prompts
    return ["Tips for getting started", "Gemini CLI", "Welcome to Gemini", "Debug Console", "❯", "> "]

def _wait_for_gemini_ready(backend, pane_id: str, *, max_wait_s: float, quiet_s: float = 0.6, poll_s: float = 0.2) -> None:
    if max_wait_s <= 0:
        return
    ...
    while time.time() < deadline:
        try:
            text = backend.get_text(pane_id, lines=80)
        except Exception:
            text = None
        if text:
            saw_output = True
            if markers and not saw_marker:
                for m in markers:
                    if m and m in text:
                        saw_marker = True
                        break
        if text != last_text:
            last_text = text
            last_change = time.time()
        ready_gate = saw_marker if markers else saw_output
        if ready_gate and (time.time() - last_change) >= quiet_s:
            return
        time.sleep(poll_s)
...
def _req_id_in_messages(messages: list[dict], req_id: str) -> bool:
    needle = f"CCB_REQ_ID: {req_id}"
    for msg in messages:
        if not isinstance(msg, dict):
            continue
...
def _wait_for_req_echo(log_reader: GeminiLogReader, req_id: str, *, timeout_s: float = 6.0, poll_s: float = 0.2) -> bool:
    if timeout_s <= 0:
        return False
    ...
    while time.time() < deadline:
        try:
            ...
            session_path = None
        if isinstance(session_path, Path) and session_path.exists():
            msgs = _read_session_messages(session_path)
            if isinstance(msgs, list) and _req_id_in_messages(msgs, req_id):
                return True
        time.sleep(poll_s)
    return False
...
class _SessionWorker(BaseSessionWorker[_QueuedTask, GaskdResult]):
    ...
    def _handle_task(self, task: _QueuedTask) -> GaskdResult:
        ...
        ready_wait_s = _cli_ready_wait_s("CCB_GASKD_READY_WAIT_S", 12.0)
        min_wait_s = _env_float("CCB_GASKD_READY_MIN_S", 20.0)
        if ready_wait_s < min_wait_s:
            ready_wait_s = min_wait_s
        if ready_wait_s > 0:
            time.sleep(ready_wait_s)

        extra_wait_s = _env_float("CCB_GASKD_READY_EXTRA_S", 8.0)
        quiet_s = _env_float("CCB_GASKD_READY_QUIET_S", 0.6)
        poll_s = _env_float("CCB_GASKD_READY_POLL_S", 0.2)
        if extra_wait_s > 0:
            _wait_for_gemini_ready(backend, pane_id, max_wait_s=extra_wait_s, quiet_s=quiet_s, poll_s=poll_s)

        send_ack_wait_s = _env_float("CCB_GASKD_SEND_ACK_WAIT_S", 6.0)
        try:
            max_attempts = int(os.environ.get("CCB_GASKD_SEND_MAX_ATTEMPTS", "2") or "2")
        except Exception:
            max_attempts = 2
        if max_attempts < 1:
            max_attempts = 1
        retry_delay_s = _env_float("CCB_GASKD_SEND_RETRY_DELAY_S", 2.0)

        for attempt in range(max_attempts):
            backend.send_text(pane_id, prompt)
            if send_ack_wait_s <= 0:
                break
            if _wait_for_req_echo(log_reader, task.req_id, timeout_s=send_ack_wait_s):
                break
            if attempt < max_attempts - 1 and retry_delay_s > 0:
