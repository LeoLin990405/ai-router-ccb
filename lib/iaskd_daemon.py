...
from iaskd_session import compute_session_key, load_project_session
from iflow_comm import IFlowLogReader, read_iflow_session_start
...
from providers import IASKD_SPEC
...
def _write_log(line: str) -> None:
    write_log(log_path(IASKD_SPEC.log_file_name), line)


def _read_iflow_session_id(session_path: Path) -> str:
...
@dataclass
class _QueuedTask:
    request: IaskdRequest
    created_ms: int
    req_id: str
    done_event: threading.Event
    result: Optional[IaskdResult] = None


class _SessionWorker(BaseSessionWorker[_QueuedTask, IaskdResult]):
    def _handle_exception(self, exc: Exception, task: _QueuedTask) -> IaskdResult:
    ...
    def _handle_task(self, task: _QueuedTask) -> IaskdResult:
        started_ms = _now_ms()
        req = task.request
        ...
        if not session:
            return IaskdResult(
                exit_code=1,
                reply="❌ No active iFlow session found for work_dir. Run 'ccb iflow' (or add iflow to ccb.config) in that project first.",
                req_id=task.req_id,
                session_key=self.session_key,
                done_seen=False,
                done_ms=None,
        ...
        log_reader = IFlowLogReader(work_dir=Path(session.work_dir))
        if session.iflow_session_path:
            try:
                log_reader.set_preferred_session(Path(session.iflow_session_path))
            except Exception:
                pass
        if session.iflow_session_id:
            log_reader.set_session_id_hint(session.iflow_session_id)
        state = log_reader.capture_state()

        try:
            session_path = state.get("session_path")
            session_id = _read_iflow_session_id(session_path) if isinstance(session_path, Path) else ""
            session.update_iflow_binding(session_path=session_path if isinstance(session_path, Path) else None, session_id=session_id or None)
                ...
                upsert_registry(
                    {
                        ...
                        "providers": {
                            "iflow": {
                                ...
                                "iflow_session_id": session.data.get("iflow_session_id"),
                                "iflow_session_path": session.data.get("iflow_session_path"),
                            }
                        },
                    }
                )
        ...
        prompt = wrap_iflow_prompt(req.message, task.req_id)
        # iFlow TUI requires pane activation before sending text for proper Enter key handling
        try:
            activate_fn = getattr(backend, "activate", None)
            if callable(activate_fn):
                activate_fn(pane_id)
                time.sleep(0.1)  # Small delay after activation
        except Exception:
            pass
        ready_wait_s = _cli_ready_wait_s("CCB_IASKD_READY_WAIT_S", 12.0)
        if ready_wait_s > 0:
        ...
        pane_check_interval = float(os.environ.get("CCB_IASKD_PANE_CHECK_INTERVAL", "2.0") or "2.0")
        ...
        while True:
            ...
            if time.time() - last_pane_check >= pane_check_interval:
                ...
                if not alive:
                    ...
                    return IaskdResult(
                        exit_code=1,
                        reply="❌ iFlow pane died during request",
                        req_id=task.req_id,
                        session_key=self.session_key,
                        done_seen=False,
                        done_ms=None,
...
class _WorkerPool:
    ...
    def submit(self, request: IaskdRequest) -> _QueuedTask:
        req_id = make_req_id()
        ...
        session_key = compute_session_key(session) if session else "iflow:unknown"
...
class IaskdServer:
    def __init__(self, host: str = "127.0.0.1", port: int = 0, *, state_file: Optional[Path] = None):
        self.host = host
        self.port = port
        self.state_file = state_file or state_file_path(IASKD_SPEC.state_file_name)
    ...
    def serve_forever(self) -> int:
        def _handle_request(msg: dict) -> dict:
            try:
                ...
                return {"type": "iask.response", "v": 1, "id": msg.get("id"), "exit_code": 1, "reply": f"Bad request: {exc}"}
            ...
            result = task.result
            if not result:
                return {"type": "iask.response", "v": 1, "id": req.client_id, "exit_code": 2, "reply": ""}
