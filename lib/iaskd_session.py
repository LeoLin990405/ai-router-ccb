...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".iflow-session")
...
class IFlowProjectSession:
    ...
    @property
    def iflow_session_id(self) -> str:
        return str(self.data.get("iflow_session_id") or "").strip()

    @property
    def iflow_session_path(self) -> str:
        return str(self.data.get("iflow_session_path") or "").strip()
    ...
    def update_iflow_binding(self, *, session_path: Optional[Path], session_id: Optional[str]) -> None:
        updated = False
        if session_path:
            try:
            ...
            if session_path_str and self.data.get("iflow_session_path") != session_path_str:
                self.data["iflow_session_path"] = session_path_str
                updated = True

        if session_id and self.data.get("iflow_session_id") != session_id:
            self.data["iflow_session_id"] = session_id
            updated = True
...
def load_project_session(work_dir: Path) -> Optional[IFlowProjectSession]:
    ...
    if data.get("active") is False:
        return None
    return IFlowProjectSession(session_file=session_file, data=data)


def compute_session_key(session: IFlowProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"iflow:{pid}" if pid else "iflow:unknown"
