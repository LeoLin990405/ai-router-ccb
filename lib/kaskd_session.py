...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".kimi-session")
...
class KimiProjectSession:
    ...
    def kimi_session_path(self) -> str:
        return str(self.data.get("kimi_session_path") or "").strip()
    ...
    def update_kimi_binding(self, *, session_path: Optional[Path], session_id: Optional[str]) -> None:
        updated = False
        if session_path:
            try:
            ...
            if session_path_str and self.data.get("kimi_session_path") != session_path_str:
                self.data["kimi_session_path"] = session_path_str
                updated = True

        if session_id and self.data.get("kimi_session_id") != session_id:
            self.data["kimi_session_id"] = session_id
            updated = True
...
def load_project_session(work_dir: Path) -> Optional[KimiProjectSession]:
    ...
    if data.get("active") is False:
        return None
    return KimiProjectSession(session_file=session_file, data=data)


def compute_session_key(session: KimiProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"kimi:{pid}" if pid else "kimi:unknown"
