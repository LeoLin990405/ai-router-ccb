"""
Kimi CLI communication module.

Reads replies from ~/.kimi/sessions/<project-hash>/<session-id>/context.jsonl and
sends prompts by injecting text into the Kimi pane via the configured backend.
"""
...
def _default_sessions_root() -> Path:
    override = (os.environ.get("KIMI_SESSIONS_ROOT") or "").strip()
    if override:
        return Path(override).expanduser()
    kimi_home = (os.environ.get("KIMI_HOME") or "").strip()
    base = Path(kimi_home).expanduser() if kimi_home else (Path.home() / ".kimi")
    return base / "sessions"


KIMI_SESSIONS_ROOT = _default_sessions_root()


def _compute_project_hash(work_dir: Path) -> str:
    """Compute Kimi's project hash (MD5 of absolute path)."""
    try:
        abs_path = str(work_dir.absolute())
...
class KimiLogReader:
    """Reads Kimi session logs from ~/.kimi/sessions"""

    def __init__(self, root: Path = KIMI_SESSIONS_ROOT, work_dir: Optional[Path] = None):
        self.root = Path(root).expanduser()
        self.work_dir = work_dir or Path.cwd()
        self._preferred_session: Optional[Path] = None
        self._session_id_hint: Optional[str] = None
        try:
            poll = float(os.environ.get("KIMI_POLL_INTERVAL", "0.05"))
        except Exception:
            poll = 0.05
    ...
    def _find_project_dir(self) -> Optional[Path]:
        """Find the Kimi project directory for current work_dir."""
        project_hash = _compute_project_hash(self.work_dir)
        if not project_hash:
            return None
        project_dir = self.root / project_hash
    ...
    def _scan_latest_session(self) -> Optional[Path]:
        ...
        try:
            for context_file in self.root.glob("*/*/context.jsonl"):
...
class KimiCommunicator:
    """Communicate with Kimi via terminal and read replies from session logs."""

    def __init__(self, lazy_init: bool = False):
        self.session_info = self._load_session_info()
        if not self.session_info:
            raise RuntimeError("❌ No active Kimi session found. Run 'ccb kimi' (or add kimi to ccb.config) first")
        ...
        self.timeout = int(os.environ.get("KIMI_SYNC_TIMEOUT", os.environ.get("CCB_SYNC_TIMEOUT", "3600")))
        self.marker_prefix = "kask"
        self.project_session_file = self.session_info.get("_session_file")

        self._log_reader: Optional[KimiLogReader] = None
        ...
        if not lazy_init:
            ...
            if not healthy:
                raise RuntimeError(f"❌ Session unhealthy: {msg}\nHint: run ccb kimi (or add kimi to ccb.config) to start a new session")

    @property
    def log_reader(self) -> KimiLogReader:
    ...
    def _ensure_log_reader(self) -> None:
        ...
        self._log_reader = KimiLogReader(work_dir=log_work_dir)
        preferred_session = self.session_info.get("kimi_session_path")
        if preferred_session:
            self._log_reader.set_preferred_session(Path(str(preferred_session)))
        session_id = self.session_info.get("kimi_session_id")
        if session_id:
    ...
    def _find_session_file(self) -> Optional[Path]:
        ...
        if env_session:
            try:
                ...
                if session_path.name == ".kimi-session" and session_path.is_file():
                    return session_path
            except Exception:
                pass
        return find_project_session_file(Path.cwd(), ".kimi-session")
    ...
    def _prime_log_binding(self) -> None:
        ...
        if not session_path:
            return
        self._remember_kimi_session(session_path)
            ...
            upsert_registry(
                {
                    ...
                    "providers": {
                        "kimi": {
                            ...
                            "kimi_session_id": self.session_info.get("kimi_session_id"),
                            "kimi_session_path": self.session_info.get("kimi_session_path"),
                        }
                    },
                }
            )
    ...
    def _remember_kimi_session(self, session_path: Path) -> None:
        ...
        if data.get("kimi_session_path") != session_path_str:
            data["kimi_session_path"] = session_path_str
            updated = True

        # Extract session ID from path (parent directory name)
        try:
            session_id = session_path.parent.name
            if session_id and data.get("kimi_session_id") != session_id:
                data["kimi_session_id"] = session_id
                updated = True
        except Exception:
            pass
            ...
            upsert_registry(
                {
                    ...
                    "providers": {
                        "kimi": {
                            ...
                            "kimi_session_id": data.get("kimi_session_id"),
                            "kimi_session_path": data.get("kimi_session_path"),
                        }
                    },
                }
            )
    ...
    def ping(self, display: bool = True) -> Tuple[bool, str]:
        ...
        msg = f"✅ Kimi connection OK ({status})" if healthy else f"❌ Kimi connection error: {status}"
        if display:
            print(msg)
        return healthy, msg
