...
class QwenProjectSession:
    ...
    def qwen_session_id(self) -> str:
        return str(self.data.get("qwen_session_id") or "").strip()

    @property
    def qwen_session_path(self) -> str:
        return str(self.data.get("qwen_session_path") or "").strip()
    ...
    def update_qwen_binding(self, *, session_path: Optional[Path], session_id: Optional[str]) -> None:
        updated = False
        if session_path:
            try:
            ...
            if session_path_str and self.data.get("qwen_session_path") != session_path_str:
                self.data["qwen_session_path"] = session_path_str
                updated = True

        if session_id and self.data.get("qwen_session_id") != session_id:
            self.data["qwen_session_id"] = session_id
            updated = True
