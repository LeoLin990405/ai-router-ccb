"""
Qwen CLI communication module.

Reads replies from ~/.qwen/projects/<project-hash>/chats/<session-id>.jsonl and
sends prompts by injecting text into the Qwen pane via the configured backend.
"""
...
def _default_projects_root() -> Path:
    override = (os.environ.get("QWEN_PROJECTS_ROOT") or "").strip()
    if override:
        return Path(override).expanduser()
    qwen_home = (os.environ.get("QWEN_HOME") or "").strip()
    base = Path(qwen_home).expanduser() if qwen_home else (Path.home() / ".qwen")
    return base / "projects"


QWEN_PROJECTS_ROOT = _default_projects_root()


def _compute_project_hash(work_dir: Path) -> str:
    """Compute Qwen's project hash (path with slashes replaced by dashes)."""
    try:
...
def _extract_message(entry: dict, role: str) -> Optional[str]:
    ...
    # Check nested message.role (Qwen format)
    # Qwen uses type="assistant" and message.role="model"
...
class QwenLogReader:
    """Reads Qwen session logs from ~/.qwen/projects"""

    def __init__(self, root: Path = QWEN_PROJECTS_ROOT, work_dir: Optional[Path] = None):
    ...
    def _find_project_dir(self) -> Optional[Path]:
        """Find the Qwen project directory for current work_dir."""
    ...
    def _scan_latest_session(self) -> Optional[Path]:
        ...
        try:
            for session_file in self.root.glob("*/chats/*.jsonl"):
                if not session_file.is_file():
                    continue
                try:
                    mtime = session_file.stat().st_mtime
...
class QwenCommunicator:
    """Communicate with Qwen via terminal and read replies from session logs."""

    def __init__(self, lazy_init: bool = False):
        self.session_info = self._load_session_info()
        if not self.session_info:
            raise RuntimeError("❌ No active Qwen session found. Run 'ccb qwen' (or add qwen to ccb.config) first")
        ...
        self.timeout = int(os.environ.get("QWEN_SYNC_TIMEOUT", os.environ.get("CCB_SYNC_TIMEOUT", "3600")))
        self.marker_prefix = "qask"
    ...
    def _ensure_log_reader(self) -> None:
        ...
        self._log_reader = QwenLogReader(work_dir=log_work_dir)
        preferred_session = self.session_info.get("qwen_session_path")
        if preferred_session:
            self._log_reader.set_preferred_session(Path(str(preferred_session)))
        session_id = self.session_info.get("qwen_session_id")
        if session_id:
    ...
    def _prime_log_binding(self) -> None:
        ...
        if not session_path:
            return
        self._remember_qwen_session(session_path)
            ...
            upsert_registry(
                {
                    ...
                    "providers": {
                        "qwen": {
                            ...
                            "qwen_session_id": self.session_info.get("qwen_session_id"),
                            "qwen_session_path": self.session_info.get("qwen_session_path"),
                        }
                    },
                }
            )
    ...
    def _remember_qwen_session(self, session_path: Path) -> None:
        ...
        if data.get("qwen_session_path") != session_path_str:
            data["qwen_session_path"] = session_path_str
            updated = True

        # Extract session ID from filename
        try:
            session_id = session_path.stem
            if session_id and data.get("qwen_session_id") != session_id:
                data["qwen_session_id"] = session_id
                updated = True
        except Exception:
            pass
            ...
            upsert_registry(
                {
                    ...
                    "providers": {
                        "qwen": {
                            ...
                            "qwen_session_id": data.get("qwen_session_id"),
                            "qwen_session_path": data.get("qwen_session_path"),
                        }
                    },
                }
            )
    ...
    def ping(self, display: bool = True) -> Tuple[bool, str]:
        ...
        msg = f"✅ Qwen connection OK ({status})" if healthy else f"❌ Qwen connection error: {status}"
        if display:
            print(msg)
        return healthy, msg
